...
public class ActivationBar : MonoBehaviour
{
    ...
    [SerializeField] private float _movingDuration=  1f;
    [SerializeField] private Ease _ease = Ease.OutCubic;
    [SerializeField] private LayerMask _attachLayerMask;
    ...
    public void ActiveMove()
    {
        ...
        _destinationPosition += _activePosition;
        _sequence.Append( 
            transform.DOMove(_destinationPosition, _movingDuration).SetEase(_ease)
        );
    ...
    {
        if (((1 << col.gameObject.layer) & _attachLayerMask) != 0)
        {
